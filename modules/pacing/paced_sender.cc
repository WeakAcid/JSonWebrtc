...
const float PacedSender::kDefaultPaceMultiplier = 1.5f;
...
PacedSender::PacedSender(const Clock* clock,
    ...
    : clock_(clock),
      ...
      time_last_process_start_ms_(0),
      time_last_process_end_ms_(0),
...
//2020-3-11 start
size_t sum_bytes_since_last_report = 0;
int64_t last_report_time = 0;
float last_report_pacing_bitrate = 0;
float PacedSender::CalculatePacingBitrateForServer(){
	int64_t now_us = clock_->TimeInMilliseconds();
        //std::cout<< " "<<std::endl;
	
        //std::cout<< " calculate pacing is  "<<sum_bytes_since_last_report * 8 <<" "<<now_us - last_report_time<<std::endl;
        if(now_us - last_report_time == 0)
	{
		return last_report_pacing_bitrate;
	}
	float report_pacing_bitrate =  static_cast<float>(sum_bytes_since_last_report * 8 / (now_us - last_report_time) * 1000);
	//std::cout<< " calculate pacing is  "<<report_pacing_bitrate << std::endl;
	last_report_time = now_us;
	sum_bytes_since_last_report = 0;
	last_report_pacing_bitrate = report_pacing_bitrate;
	return report_pacing_bitrate;
}
//2020-3-11 end
...
int64_t PacedSender::ExpectedQueueTimeMs() const {
  ...
  //std::cout<<"ExpectedQueueTimeMs "<<packets_->SizeInBytes() * 8 / pacing_bitrate_kbps_<<std::endl;
...
void PacedSender::Process() {
  ...
  if (send_padding_if_silent_ || paused_ || Congested()) {
	...
	sum_bytes_since_last_report += bytes_sent;
      }
    }
  }
  if (paused_)
  ...
  if (packets_->Empty() && !Congested()) {
    ...
    if (packet_counter_ > 0) {
      ...
      //int padding_needed = estimated_bitrate_bps_ / 1000 * kMinPacketLimitMs / 8 - bytes_sent;
      if (padding_needed > 0) {
  ...
  //std::cout<<"QueueTime ms "<<ExpectedQueueTimeMs()<<std::endl;
  //std::cout<<"packets_->SizeInBytes() "<<packets_->SizeInBytes()<<std::endl;
  //process_end = clock_->TimeInMicroseconds();
  sum_bytes_since_last_report += bytes_sent;
  //std::ofstream process_file("process_frequency.txt",std::ios::app);
  //process_file<<process_start<<" "<<process_end<<" "<<bytes_sent<<" "<<estimated_bitrate_bps_/1000<<" "<<ExpectedQueueTimeMs()<<std::endl;
  //process_file<<process_start<<" "<<process_end<<" "<<bytes_sent<<" "<<estimated_bitrate_bps_/1000<<" "<<media_sent<<" "<<isempty<<" "<<ispadding<<" "<<packets_->SizeInBytes()<<std::endl;
  //process_file.close();
}
...
bool PacedSender::SendPacket(const PacketQueueInterface::Packet& packet,
  ...
  //if(pacing_info.probe_cluster_id == PacedPacketInfo::kNotAProbe)
	//std::cout<<"pacing_info.probe_cluster_id == PacedPacketInfo::kNotAProbe"<<std::endl;
  //if(media_budget_->bytes_remaining() == 0)
	//std::cout<<"media_budget_->bytes_remaining() == 0"<<std::endl;
