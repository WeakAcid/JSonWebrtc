...
class PacedSender : public Pacer {
 public:
  ...
  //2020-3-11 start
  float CalculatePacingBitrateForServer();
  //2020-3-11 end
 ...
 private:
  ...
  size_t value_ RTC_GUARDED_BY(critsect_);

  int64_t time_last_process_us_ RTC_GUARDED_BY(critsect_);
  int64_t time_last_process_start_ms_ RTC_GUARDED_BY(critsect_);
  int64_t time_last_process_end_ms_ RTC_GUARDED_BY(critsect_);
